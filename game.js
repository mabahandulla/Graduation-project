...
class Level {
  ...
  obstacleAt(posTo, size) {
    ...
    const rigth = Math.floor(x + xSize);
    const middle = Math.round(top + ySize / 2);

    if (this.checkObstacle(left, top)) {
      return this.grid[top][left];
    }
    if (this.checkObstacle(rigth, top)) {
      return this.grid[top][rigth];
    }
    if (this.checkObstacle(left, bottom)) {
      return this.grid[bottom][left];
    }
    if (this.checkObstacle(rigth, bottom)) {
      return this.grid[bottom][rigth];
    }
    if (this.checkObstacle(left, middle)) {
      return this.grid[middle][left];
    }
    if (this.checkObstacle(rigth, middle)) {
      return this.grid[middle][rigth];
    }
  ...
  noMoreActors(type) {
    const noActors = this.actors.filter(item => item.type === type);
    return noActors.length > 0 ? false : true;
  }
...
class LevelParser{
  ...
  obstacleFromSymbol(symbol){
   ...
   if(symbol !== 'x' || symbol !== '!') return undefined;
  } 
